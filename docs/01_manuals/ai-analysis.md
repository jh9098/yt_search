...
## 보정(Fallback) 규칙 예시
AI 출력이 부분적으로 깨졌을 때, 전체 실패 대신 일부 보정을 허용할 수 있습니다.

예시:
- `recommendedKeywords` 누락 → `[]`
- `contentIdeas` 누락 → 빈 배열 + warning meta 추가
- `summary.weakPoints` 누락 → "충분한 데이터가 없어 아쉬운 점을 특정하기 어렵습니다."

주의:
- 보정 규칙은 일관되어야 하며 문서화해야 함
- 임의 보정으로 의미 왜곡 금지
...
### 부분 성공 허용 정책 (권장)
댓글 수집 실패 시에도 제목/설명/메타 기반으로 "축소 분석"을 수행할지 결정 필요

예:
- `analysisBasis: ["title", "description"]`
- `warnings: ["댓글 데이터를 수집하지 못해 축소 분석으로 진행됨"]`
